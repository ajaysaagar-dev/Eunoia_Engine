...
export default function ViewportCamera() {
    ...
    EngineRegistry.EunoiaEngine_Camera_TN = new TransformNode('Viewport_Camera_Parent', EngineRegistry.EunoiaEngine_Scene as Scene);
    EngineRegistry.EunoiaEngine_Camera = new FreeCamera(
        'viewport_camera',
        new Vector3(0, 3, -8),
        EngineRegistry.EunoiaEngine_Scene as Scene
    );

    (EngineRegistry.EunoiaEngine_Camera as FreeCamera).parent = EngineRegistry.EunoiaEngine_Camera_TN as TransformNode;
    (EngineRegistry.EunoiaEngine_Camera_TN as TransformNode).position.set(0, 2, -10);
...
export function ViewportControlsDisable() {
    ...
    if (EngineRegistry.EunoiaEngine_Renderers) {
        EngineRegistry.EunoiaEngine_Renderers =
            EngineRegistry.EunoiaEngine_Renderers.filter((renderer: any) => renderer.name !== 'ViewportControlsUpdate');
        EngineRegistry.EunoiaEngine_Renderers =
            EngineRegistry.EunoiaEngine_Renderers.filter((renderer: any) => renderer.name !== 'ViewportLooksControlsUpdate');
    }
}
...
function ViewportLooksControlsUpdate() {
    if (!looking) return;
    const delta = (EngineRegistry.EunoiaEngine_Engine as Engine)?.getDeltaTime() ?? 0;
    if (EngineRegistry.EunoiaEngine_Camera_TN) {
        (EngineRegistry.EunoiaEngine_Camera_TN as FreeCamera).rotation.y += looks.x * 0.001 * delta;
        (EngineRegistry.EunoiaEngine_Camera_TN as FreeCamera).rotation.x += looks.y * 0.001 * delta;
    }
...
function ViewportControlsUpdate() {
    ...
    if (!camTN) return;
    if (moves.w)
        camTN.translate(Vector3.Forward(), 0.1, Space.LOCAL);
    if (moves.a)
        camTN.translate(Vector3.Left(), 0.1, Space.LOCAL);
    if (moves.s)
        camTN.translate(Vector3.Backward(), 0.1, Space.LOCAL);
    if (moves.d)
        camTN.translate(Vector3.Right(), 0.1, Space.LOCAL);
    if (moves.q)
        camTN.translate(Vector3.Down(), 0.1, Space.WORLD);
    if (moves.e)
        camTN.translate(Vector3.Up(), 0.1, Space.WORLD);
}
